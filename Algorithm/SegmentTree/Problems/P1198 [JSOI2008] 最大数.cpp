...
struct segmentTree {
	...
	void push_up(int i) {
		int a = i << 1, b = i << 1 | 1;
		T mx = max(tr[a].mx, tr[b].mx);

		mx %= mod;
		if(tr[i].mx < mx) {
			if(tr[a].mx < tr[b].mx) swap(a, b);
			tr[i].mx = mx;
			tr[i].ind1 = tr[a].ind1;
		}
	}
	...
	T query1(int i, int ql, int qr) {
		...
		push_down(i);
		return max(query1(i << 1, ql, qr), query1(i << 1 | 1, ql, qr)) % mod;
	}
...
signed main() {
    ...
    while(m--) {
    	...
    	if(op == 'Q') {
    		int l;
    		cin >> l;
    		mx = seg.query1(len - l, len - 1);
    		cout << mx << '\n';
    	} else if(op == 'A') {
    		ll n;
    		cin >> n;
    		len++;
    		seg.update1(len - 1, mx + n);
    	}
    }
